...
import { 
  Code2, ClipboardCheck, Sparkles, Globe, 
  Image as ImageIcon, Zap, Sun, Moon, Copy, Check, Terminal, FileDown, Trash2
} from 'lucide-react';
...
function App() {
  ...
  return (
    <div className="flex h-screen w-full font-sans transition-colors duration-300 dark:bg-[#0a0a0a] bg-slate-50 overflow-hidden relative">
      
      {/* GLOBAL TOAST (Fixed position to avoid button overlap) */}
      {copied && (
        <div className="fixed top-20 left-1/2 -translate-x-1/2 z-[100] bg-indigo-600 text-white text-xs font-bold px-6 py-2.5 rounded-full shadow-2xl flex items-center gap-2 animate-in">
          <Check size={14} /> Copied to Clipboard
        </div>
      )}

      {/* Sidebar */}
      <aside className="w-80 border-r dark:border-zinc-800 border-slate-200 dark:bg-zinc-950 bg-white flex flex-col p-6 z-20 overflow-y-auto">
        ...
        <div className="space-y-8 flex-1">
          ...
          {/* QA ARCHITECT CARD */}
          <div className="p-4 bg-indigo-500/5 border border-indigo-500/10 rounded-xl mt-6">
            ...
            <p className="text-[10px] leading-relaxed dark:text-zinc-400 text-slate-500">
              For enterprise reliability, save POM classes in <span className="font-mono text-indigo-400">/models</span> and test specs in <span className="font-mono text-indigo-400">/tests</span>.
            </p>
          </div>
        </div>
      </aside>
      ...
      {/* Main Workspace */}
      <main className="flex-1 flex flex-col relative dark:bg-zinc-950 bg-white">
        <header className="h-16 border-b dark:border-zinc-800 border-slate-200 flex items-center justify-between px-8 bg-inherit z-50">
          <div className="flex items-center gap-2 text-xs font-semibold dark:text-zinc-500 text-slate-400">
            <Terminal size={14} /> <span>gherkin-to-playwright.ts</span>
          </div>
          <button 
            onClick={handleConvert}
            disabled={loading || !input}
            className="bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-2 rounded-lg text-xs font-bold transition-all shadow-xl shadow-indigo-500/20 active:scale-95 disabled:opacity-50 flex items-center gap-2"
          >
            {loading ? <span className="animate-pulse">Analyzing...</span> : <><Sparkles size={14} /> Generate Code</>}
          </button>
        </header>

        <div className="flex-1 flex overflow-hidden">
          <div className="flex-1 p-0 relative border-r dark:border-zinc-800 border-slate-200">
            <textarea 
              className="w-full h-full dark:bg-[#0d0d0d] bg-white p-10 outline-none dark:text-zinc-200 text-slate-700 font-mono text-sm leading-relaxed resize-none"
              placeholder="Scenario: User can Login..."
              value={input} onChange={(e) => setInput(e.target.value)}
            />
          </div>

          <div className="flex-1 p-0 relative dark:bg-[#080808] bg-slate-100 group overflow-auto">
            ...
            <pre className="w-full h-full p-10 font-mono text-[13px] leading-relaxed !bg-transparent text-left">
